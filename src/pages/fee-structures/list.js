...
export default function FeeStructureDataTable() {
  ...
  const [sortConfig, setSortConfig] = useState({ key: 'feeType', direction: 'ascending' });
  ...
  // --- DATA FETCHING (PAGINATED FEE STRUCTURES) ---
  const fetchPageData = useCallback(async (page, limit, search, sort) => {
    ...
    try {
      let fetchedFeeStructures = [];
      let totalCount = 0;

      if (searchLower) {
        // Client-side exact and fuzzy search across all fields
        const allFeeStructures = Data.feeStructures.list() || [];
        const fuse = new Fuse(allFeeStructures, {
          keys: [
            "feeType",
            "description",
            "amount",
        ...
        fetchedFeeStructures = sortedResults.slice(startIndex, endIndex);

      } else {
        // Server-side pagination for default view
        const pageResponse = await Data.feeStructures.getPage({
          page,
          limit,
          search: "",
          sort,
        ...
        fetchedFeeStructures = pageResponse.feeStructures;
        totalCount = pageResponse.totalCount;
      }
      
      setFeeStructures(fetchedFeeStructures);
      setTotalFeeStructures(totalCount);
    } catch (error) {
      console.error("Failed to fetch fee structure page:", error);
      setFeeStructures([]);
      setTotalFeeStructures(0);
    } finally {
  ...
  // Effect to fetch PAGE data (Fee Structures)
  useEffect(() => {
  ...
  const headers = useMemo(() => [
      ...
      { key: 'term.name', label: 'Term', sortable: true },
      { key: 'isRequired', label: 'Required', sortable: true },
      { key: 'isActive', label: 'Active', sortable: true },
  ], []);

  // --- DERIVED STATE ---
  const totalPages = Math.ceil(totalFeeStructures / rowsPerPage);
  ...
  const handleFeeStructureCreated = (newFeeStructure) => {
    if (!newFeeStructure || !newFeeStructure.id) return;
    setCurrentPage(1);
    setFeeStructures(prev => [newFeeStructure, ...prev.slice(0, rowsPerPage - 1)]);
    setTotalFeeStructures(prev => prev + 1); 

    setNewlyAddedIds(prev => new Set(prev).add(newFeeStructure.id));
    const timerId = setTimeout(() => {
        setNewlyAddedIds(prev => {
            const newIds = new Set(prev);
            newIds.delete(newFeeStructure.id);
            return newIds;
        });
        newRecordTimers.current.delete(newFeeStructure.id);
    }, 5000);
    newRecordTimers.current.set(newFeeStructure.id, timerId);
  };

  const handleCreateFeeStructure = async (feeStructureData) => {
    try {
        const newFeeStructure = await Data.feeStructures.create(feeStructureData);
        handleFeeStructureCreated(newFeeStructure);
    } catch (error) {
    ...
    <div className="v8-datatable-container">
      ...
      <header className="v8-header">
        <div className="v8-header-title-container">
            <h2 className="v8-header-title">Fee Structures</h2>
            <p className="v8-header-desc">Manage fee structures, amounts, and payment requirements for classes and terms.</p>
        </div>
      ...
      <main className="v8-main">
        ...
        <div className="v8-toolbar">
            <div className="v8-search-group">
                <input type="text" className="v8-search-input" placeholder="Search fee structures by type, description, class, term, amount..." value={searchTerm} onChange={(e) => handleRealTimeSearch(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} />
                    ...
                    <th key={h.key} className={h.sortable ? 'sortable' : ''} onClick={() => h.sortable && requestSort(h.key)}>
                        ...
                        {h.sortable && (
                            <span className={`sort-icon ${sortConfig.key === h.key ? 'active' : ''}`}>
                                {sortConfig.key === h.key && sortConfig.direction === 'ascending' ? 'ArrowUp' : 'ArrowDown'}
                            </span>
                        )}
                    </th>
                ))}
                    ...
                    <tr key={row.id} className={newlyAddedIds.has(row.id) ? 'v8-new-row' : ''}>
                      {headers.map(h => {
                        ...
                        if (h.key === 'amount') {
                          ...
                          const labelText = isActive ? 'Yes' : 'No';
                          value = <span className={`v8-badge ${badgeClass}`}>{labelText}</span>;
                        }
                        return <td key={h.key} className={h.key === 'feeType' ? 'td-primary' : ''}>{value}</td>;
                      })}
                      <td className="v8-table-actions" style={{textAlign: 'right'}}>
                        <button className="v8-tooltip-container" onClick={() => handleEdit(row)}>
                            ...
                            <span className="v8-tooltip-text">Edit Fee Structure</span>
                        </button>
        ...
        {!initialLoading && totalFeeStructures > 0 && (
          <div className="v8-pagination">
            <div className="v8-pagination-info">
                Showing <strong>{(currentPage - 1) * rowsPerPage + 1}</strong>-<strong>{Math.min(currentPage * rowsPerPage, totalFeeStructures)}</strong> of <strong>{totalFeeStructures}</strong>
            </div>
