...
class Delete extends Component {
    ...
    handleDelete = async () => {
        ...
        try {
            await Data.budgets.delete(this.props.item);
            if(window.toastr) window.toastr.success("Budget deleted successfully!");
            this.props.close();
        } catch (error) {
            console.error(error);
            if(window.toastr) window.toastr.error("Failed to delete Budget.");
        } finally {
        ...
        return (
            <div className="modal-content border-0">
                <div className="modal-header bg-danger">
                    <h5 className="modal-title font-weight-bolder text-white">Delete Budget</h5>
                ...
                <div className="modal-body text-center py-5">
                    ...
                    <p className="text-muted">
                        You are about to delete the <strong>{item.name}</strong> budget. This action cannot be undone.
                    </p>
                </div>
